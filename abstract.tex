...
Nós apresentamos avanços metodológicos na área de modelagem direta e inversão
regional de dados de gravimetria por satélite.
Com esse fim, nós desenvolvemos dois projetos de software de código livre.
...
adaptativa para garantir a acurácia das computações automaticamente.
Nossos resultados com testes numéricos mostram que, para obter o mesmo nível de
acurácia, a aceleração gravitacional demanda uma discretização mais fina que o
potencial.
Por sua vez, o tensor gradiente gravitacional demanda discretização mais final
ainda que para a aceleração.
O segundo software é o \textit{Fatiando a Terra}, uma biblioteca feita na
linguagem Python para inversão, modelagem direta, processamento e visualização
de dados.
A biblioteca permite que o usuário combine as ferramentas de modelagem direta e
inversão para implementar novos métodos de inversão.
As ferramentas de modelagem direta incluem uma implementação do algoritmo
utilizado no programa \textit{Tesseroids}.
Nós combinamos os recursos de inversão e modelagem direta com tesseroides do
...
dados de gravidade utilizando uma aproximação esférica para a Terra.
Nós adaptamos o método de Bott, que é computacionalmente eficiente, para
...
regularização, o contraste de densidade na Moho e a profundidade da Moho de
referência.
Nós aplicamos dois tipos de validação cruzada para estimar esses parâmetros de
maneira automática.
...
Por fim, nós aplicamos nosso método de inversão para gerar um modelo de
profundidades da Moho para a América do Sul.
Nosso modelo ajusta os dados de gravidade observados e estimativas da
profundidade da Moho provenientes da sismologia
nas regiões oceânicas e nas partes central e Leste do continente.
Nós observamos desajustes aos dados na região dos Andes, onde a profundidade da
Moho é a maior para o continente.
