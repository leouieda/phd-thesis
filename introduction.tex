...
inverse problem \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{backus1967, backus1968}
}%DIFAUXCMD
}\DIFaddend .
...
usually through \DIFdelbegin \DIFdel{regularization}\DIFdelend \DIFaddbegin \DIFadd{Tikhonov regularization \mbox{%DIFAUXCMD
\citep{tikhonov1977}
}%DIFAUXCMD
}\DIFaddend .
Different regularizing functions favor different model attributes, such as
smoothness or compactness \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[e.g., ][]{silva2001b}
}%DIFAUXCMD
}\DIFaddend .
...
application \DIFdelbegin \DIFdel{all three components of an inversion method (}\DIFdelend \DIFaddbegin \DIFadd{three modules: }\DIFaddend the forward problem, \DIFdelbegin \DIFdel{optimization, and regularization)}\DIFdelend \DIFaddbegin \DIFadd{an optimization algorithm, and
a regularizing function}\DIFaddend .
Fortunately, these \DIFdelbegin \DIFdel{components }\DIFdelend \DIFaddbegin \DIFadd{modules }\DIFaddend can usually be uncoupled.
For example, the implementation of the forward problem does not depend on the
choice of optimization method or \DIFdelbegin \DIFdel{regularization}\DIFdelend \DIFaddbegin \DIFadd{regularizing function}\DIFaddend .
Likewise, the software implementation of \DIFdelbegin \DIFdel{optimization }\DIFdelend \DIFaddbegin \DIFadd{an optimization algorithm }\DIFaddend requires
only a knowledge of a function to \DIFdelbegin \DIFdel{optimize }\DIFdelend \DIFaddbegin \DIFadd{be optimized }\DIFaddend (and possibly its derivatives),
...
These three \DIFdelbegin \DIFdel{components }\DIFdelend \DIFaddbegin \DIFadd{modules }\DIFaddend can be combined in different ways to produce
new inversion software.

Here, we develop two software projects and apply their reusable \DIFdelbegin \DIFdel{components }\DIFdelend \DIFaddbegin \DIFadd{modules }\DIFaddend to
